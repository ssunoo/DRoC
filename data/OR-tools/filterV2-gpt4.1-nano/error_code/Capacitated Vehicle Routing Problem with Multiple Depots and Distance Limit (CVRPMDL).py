...
def solve(distance_matrix: list, demands: list, num_vehicle: int, vehicle_capacities: list, depot: int, starts: list, ends: list, distance_limit: int):
    ...
    # Create Routing Model, which is the main data structure for defining the VRP.
    ...
    # This dimension keeps track of the cumulative distance traveled by each vehicle.
    distance_dimension_name = 'Distance'
    routing.AddDimension(
        transit_callback_index,
        0,  # no slack, meaning no extra distance allowed beyond the distance matrix.
    ...
    # This callback returns the demand at a given node.
    ...
    # It tracks the cumulative demand along each route.
    routing.AddDimensionWithVehicleCapacity(
        demand_callback_index,
        0,  # no slack, demand cannot be exceeded.
