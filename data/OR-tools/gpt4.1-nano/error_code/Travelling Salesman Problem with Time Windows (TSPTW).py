...
def solve(time_matrix: list, time_windows: list, depot: int):
    ...
    if solution:
        obj = routing.GetArcCostForVehicle(manager.NodeToIndex(depot), manager.NodeToIndex(depot), 0)
        # Alternatively, get total time from the time dimension
        total_time = time_dimension.CumulVar(manager.NodeToIndex(depot)).Max()
        return total_time
    else:
        return -1